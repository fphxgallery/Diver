...
- **Meteora DLMM** — Browse pools sorted by 24h Fee/TVL ratio with token logos, open positions with Spot/Curve/Bid-Ask strategies, remove liquidity, and claim fees. Filter pools by minimum TVL from Settings. Pool address links directly to Meteora. Pool detail shows TVL, 24h volume, fees, Fee/TVL, APY, and current price. Liquidity Distribution chart shows per-bin token X/Y breakdown with stacked color-coded bars.
- **Position detail** — Each position card shows token amounts with live USD values, unclaimed fees, and an in-range progress bar. Transaction errors surface inline. Solscan link auto-clears after 10 seconds.
